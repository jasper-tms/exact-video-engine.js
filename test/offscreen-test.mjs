// Drives test-offscreen.html through the browser named by TEST_BROWSER (chromium,
// webkit, or firefox): a host that never puts the engine's canvas in the
// document, and only wants pixels out of it. The WebCodecs tier assertion holds
// on every engine that has WebCodecs, so there are no per-browser expectations.
//
...
for (const { mode } of CASES) {
  ...
  await page.goto(`${serverBase}/test/test-offscreen.html?file=${file}&mode=${mode}`);
